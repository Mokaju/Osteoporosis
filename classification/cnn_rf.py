...
def main(
    ...
    save_dir='Binary-classification models/CNN-RF'
):
    ...
    for m, path in enumerate(cnn_paths):
        ...
        rf_classifier.fit(cnn_x_train, y_train)

        y_pred = rf_classifier.predict(cnn_x_test)

        joblib.dump(
            rf_classifier,
            os.path.join(save_dir, f'rf_classifier_model_for_fold{m}.pkl')
        )

        cm, metrics = compute_binary_metrics(y_test, y_pred)
        pd.DataFrame(cm).to_csv(
            os.path.join(save_dir, f'rf_classifier_confusion_matrix_for_fold{m}.csv'),
            index=False
        )
        pd.DataFrame([metrics]).to_csv(
            os.path.join(save_dir, f'CNN-RF_metrics_fold_{m}_for_binary_classes.csv'),
            index=False
        )
